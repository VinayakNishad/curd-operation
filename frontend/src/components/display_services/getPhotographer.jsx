  ...
  useEffect(() => {
    const fetchStudios = async () => {
      try {
        ...
        // console.log("From GetStudios:" + service);

        const serviceData = { "service": service };
        const response = await axios.get("http://localhost:3001/getStudios", { params: serviceData });
          ...
          {studios.map((studio) => (
            <div className="studio-card-wrapper">
              <div className="edit-btn-wrapper">
                {
                  props.admin ?
                    <button className="edit-btn" onClick={() => { handleShow(); setCurrentStudio(studio) }}>Edit</button> : ""
                }
              </div>
              <Link
